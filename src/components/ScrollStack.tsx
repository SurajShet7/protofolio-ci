...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    const currentLenisRef = lenisRef;
    const currentStackCompletedRef = stackCompletedRef;
    const currentCardsRef = cardsRef;
    const currentInitialTopsRef = initialTopsRef;
    const currentLastTransformsRef = lastTransformsRef;
    const currentIsUpdatingRef = isUpdatingRef;

    return () => {
      if (currentAnimationFrameRef.current) {
        cancelAnimationFrame(currentAnimationFrameRef.current);
      }
      if (currentLenisRef.current) {
        currentLenisRef.current.destroy();
      }
      currentStackCompletedRef.current = false;
      currentCardsRef.current = [];
      currentInitialTopsRef.current = [];
      currentLastTransformsRef.current.clear();
      currentIsUpdatingRef.current = false;
    };
  }, [
    itemDistance,
    itemScale,
